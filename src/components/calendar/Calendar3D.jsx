...
export default function Calendar3D({ tokens, isMobile = false, isTablet = false, isTabletLandscape = false }) {
  ...
  return (
    <group position={[0, 3.18, 0.5]}>
      ...
      {showHolidaysPanel && (isMobile || isTablet) ? (
        <group position={[0, isTablet ? -1.45 : -1.3, 0.05]}>
          <mesh>
            <planeGeometry args={[isTablet ? 2.3 : 1.95, isTablet ? 2.2 : 1.8]} />
            <meshStandardMaterial color={tokens.calendarBg || '#1a212c'} roughness={0.9} transparent opacity={0.95} />
          </mesh>
          <Html transform position={[0, 0, 0.01]} distanceFactor={1.5} style={{ width: isTablet ? '250px' : '210px', pointerEvents: 'none' }}>
            <div style={{
              padding: isTablet ? '16px' : '14px',
              fontFamily: 'Manrope, sans-serif',
              color: tokens.calendarText || '#ebf2fb',
              fontSize: isTablet ? '17px' : '16px',
              lineHeight: '1.5'
            }}>
              <div style={{ fontWeight: 'bold', fontSize: isTablet ? '20px' : '18px', marginBottom: '12px', borderBottom: `3px solid ${tokens.calendarAccent}`, paddingBottom: '8px' }}>
                Holidays
              </div>
              {currentMonthHolidays.length === 0 ? (
                <div style={{ opacity: 0.6, fontStyle: 'italic', fontSize: isTablet ? '15px' : '14px' }}>No holidays this month</div>
              ) : (
                currentMonthHolidays.map((h, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', alignItems: 'center' }}>
                    ...
                    <span style={{ flex: 1, marginLeft: '14px', fontSize: isTablet ? '15px' : '14px', fontWeight: '600' }}>{h.name}</span>
                  </div>
                ))
              )}
